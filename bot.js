...
bot.onText(/^\/key (.+)/, (msg, m) => {
  ...
  if (key === ADMIN_KEY) {
    users[id] = { role: 'admin', key, usedKeys: [key], history: {} };
    saveUsers();
  ...
  users[id] ||= { role: 'user', usedKeys: [], history: {} };
  users[id].key = key;
  if (!users[id].usedKeys.includes(key)) users[id].usedKeys.push(key);
...
bot.onText(/^\/userinfo (\d+)/, (msg, m) => {
  const id = msg.chat.id;
  if (!isAdmin(id)) return;

  const u = users[m[1]];
  if (!u) return bot.sendMessage(id, '❌ Không tìm thấy user');
...
for (let i = 1; i <= 16; i++) {
  ...
  bot.onText(new RegExp(`^/${table.toLowerCase()}$`), async (msg) => {
    const id = msg.chat.id;
    if (!isLogin(id)) return bot.sendMessage(id, '🔐 Nhập /key trước');
